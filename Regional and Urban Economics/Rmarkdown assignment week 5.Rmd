...
#converting a tibble into a dataframe and then making a table with stargazer with the descriptive statistics
df_utrecht_clean_df <- as.data.frame(df_utrecht_clean)
stargazer(df_utrecht_clean_df, type = "text")
...
#Converting a tibble into a dataframe and using stargazer to create a table with the descriptive statistics of the independent variables we will use the regression model
subdata_nl = as.data.frame(subdata_nl)
stargazer(subdata_nl, type = "text")
...
stargazer(model_basic_utrecht, model_basic_national, type = "text", column.labels = c("Utrecht", "The Netherlands"))
...
model_basic_national <- lm(lnpricem2 ~ distcbd, data = subdata_nl)
model_extended_national <- lm(lnpricem2 ~ distcbd + popdens + shforeign+shlandinfr + shopenspace + shlandres + shyoungp + shelderly, data = subdata_nl)

# Create a table with the regression outcomes
stargazer(model_basic_utrecht, model_extended_utrecht, model_basic_national, model_extended_national, type = "text")

stargazer(model_extended_national, type = "text")

complete_model = list(model_basic_utrecht, model_extended_utrecht, model_basic_national, model_extended_national)

stargazer(complete_model, type = "text", column.labels = c("Utrecht, Basic", "Utrecht, Extended", "The Netherlands, Basic", "The Netherlands, Extended"))
```
